...
foreach ($edupage_lessons as $edupage_lesson) {
    // Robust decode of `student` field to avoid warnings when NULL/invalid
    $lesson_json = $edupage_lesson['student'] ?? '[]';
    if ($lesson_json === null || $lesson_json === '' || strtolower((string)$lesson_json) === 'null') {
        $lesson_json = '[]';
    }
    $lesson_students = json_decode($lesson_json, true);
    if (!is_array($lesson_students)) {
        $lesson_students = [];
    }
    foreach ($lesson_students as $lesson_student) {
        ...
        // attendance_status may be missing in some records
        $student_data_single['attendance_status'] = $lesson_student['attendance_status'] ?? null;
...
foreach ($student_data as $row) {
    ...
    if ($row['attendance_status'] === false ) {
        $students[$key]['countAbsolvedLessons']++;
    } elseif ($row['attendance_status'] === 0 || $row['attendance_status'] === '0') {
        $students[$key]['countPlannedLessons']++;
