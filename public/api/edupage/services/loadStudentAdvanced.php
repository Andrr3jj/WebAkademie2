...
function loadStudentsAdvanced($id) {
    ...
    $data['points'] = $data['starsCount'];

    for($y = 2025; $y < 2030; $y++) {
        for($m = 1; $m < 12; $m++){
            $lesson_count = 0;
            $sql = "SELECT * FROM edupage_lesson WHERE date LIKE '%.$m.$y' AND student LIKE '%$search%' OR student LIKE '%$search_1%'";
            $res = mysqli_query($conn, $sql);
            if (!mysqli_num_rows($res)) continue;
            while ($row = mysqli_fetch_assoc($res)) {
                if (str_contains($row['status'], "canceled") and $lesson_count < 4) $lesson_count++;
            }
            if ($lesson_count == 4) $data['points'] += 2;
        }
    }
