...
function loadMyLessonAll($student_id) {
    ...
    $search_0 = 'student_id":' . $student_id . '"';
    $search_1 = 'student_id":' . $student_id . ",";
...
function cancelLesson($lesson_id) {
    ...
    $lesson = loadLesson($lesson_id)[0];
    $lesson_student = json_decode($lesson['student'], true);

    if ($lesson_student[0]['attendance_status'] != "false") $lesson_student[0]['attendance_status'] = "false";
    else $lesson_student[0]['attendance_status'] = "true";
    if (count($lesson_student) > 1) {
        if ($lesson_student[1]['attendance_status'] != "false") $lesson_student[1]['attendance_status'] = "false";
        else $lesson_student[1]['attendance_status'] = "true";
    }

    $lesson_student = json_encode($lesson_student);

    $sql = "UPDATE edupage_lesson SET status='canceled', student='$lesson_student' WHERE id='$lesson_id'";
    if (mysqli_query($conn, $sql)) return true;
    return false;
}
