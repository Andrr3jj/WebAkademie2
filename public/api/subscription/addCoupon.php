...
if (isset($_SESSION['logged_in'])) {
    if (!empty($_GET['code']) ) {
        ...
        $sql = "SELECT * FROM coupon_subscription WHERE code='$code' AND deleted=false AND valid_until > '$time_now'";
        $result = mysqli_query($conn, $sql);
        
	    if (mysqli_num_rows($result) == 1) {
            while($row = mysqli_fetch_assoc($result)) {
                if (empty($_SESSION['cart_subscription_discount_coupon'])) $_SESSION['cart_subscription_discount_coupon'] = array();
                if (!in_array(json_encode($row), $_SESSION['cart_subscription_discount_coupon'])) {
                    if ($row['uses_left'] > 0 and $row['deleted'] == false and $row['valid_until'] > time()) {
                        array_push($_SESSION['cart_subscription_discount_coupon'], json_encode($row));
                        response("Request fullfiled");
                    } else response("Request failed", "Already used or expired or deleted");
                } else response("Request failed", "Already in cart_subscription");
            }
