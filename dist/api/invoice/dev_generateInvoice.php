...
$testData = [
    ...
    "customer_name" => "Andrej Trnka",
    "customer_address" => [
        "Krasňany 103,",
        "01303 Krasňany Slovensko",
        "Slovenská republika"
    ],

    "products" => [
        [
            "name" => "Číselný zápis:\nAch mamičko mamko moja",
            "qty" => 1,
            "price" => "2.00",
        ],
        [
...
if (isset($_SESSION['logged_in'])) {
    class PDF extends tFPDF {
        function body($data) {
            ...
            foreach ($data['products'] as $row) {
                ...
                $this->Cell(30, 5, "  " . dot2comma($row["price"]), "", 0, "C");
                $this->Cell(20, 5, number_format($row["qty"] * $row["price"], 2, ",") . "     ", "", 1, "R");
                $product_count++;
    ...
    //$pdf->Output('F', 'invoices/'.$testData['invoice_no'].'.pdf');
    $pdf->Output();

} else $pdf->Write(10, 'Zobrazovanie faktúr nie je povolené neprihlaseným uživateľom!');
