...
function sendMail_edupagePaymentRequestSendEmail($email, $data) {
    ...
    try {
        ...
        //Content
        $mailBody = file_get_contents(__DIR__ . "/edupagePaymentRequest.html");
        ...
        */
        ...
        $altBody = file_get_contents(__DIR__ . "/edupagePaymentRequest.txt");
        //$altBody = str_replace("%data%", $data['data'], $altBody);


        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = $mail_config['subject_edupagePaymentRequest'];
        $mail->Body    = $mailBody;
        ...
        fwrite($logfile, "\r\n". time() . "edupagePaymentRequest.php - ErrorMailerError:{$mail->ErrorInfo} (e={$e})");
        fclose($logfile);
