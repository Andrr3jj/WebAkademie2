...
function sendMail_forgotPasswordSendEmail($email, $recoveryKey, $name, $surname) {
    ...
    try {
        ...
        //Content
        $mailBody = str_replace("%recoveryKey%", $recoveryKey, file_get_contents(__DIR__ . "/forgotPasswordSendEmail.html"));
        $mailBody = str_replace("%name%", $name, $mailBody);
        $mailBody = str_replace("%surname%", $surname, $mailBody);
        
        $altBody = str_replace("%recoveryKey%", $recoveryKey, file_get_contents(__DIR__ . "/forgotPasswordSendEmail.txt"));
        $altBody = str_replace("%name%", $name, $altBody);
        $altBody = str_replace("%surname%", $surname, $altBody);

        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = $mail_config['subject_forgotPassword'];
        $mail->Body    = $mailBody;
        ...
        fwrite($logfile, "\r\n". time() . "forgotPasswordSendEmail.php - ErrorMailerError:{$mail->ErrorInfo}");
        fclose($logfile);
