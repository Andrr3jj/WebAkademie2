<template>
  <div class="text-piesne" v-if="songDataInternal">
    ...
    <ZapisGrid :produkty="dalsieProdukty" />
  </div>
...
export default {
  name: "TextPiesneNeaktivny",
  components: { ZapisGrid },
  props: {
    songDetail: {
      type: Object,
      default: null,
  ...
  computed: {
    rozdelText() {
      if (!this.songDataInternal?.text_piesne) return [];
  ...
  async mounted() {
    // ak sme už fetchovali alebo ešte nevieme songDetail.id, skončíme
    if (this.fetchedOnce || !this.songDetail?.id) return;
    this.fetchedOnce = true;

    // kópia detailu piesne (neovplyvní originál)
    this.songDataInternal = JSON.parse(JSON.stringify(this.songDetail));

    // ✅ prirátanie zobrazenia
    try {
      await fetch("https://heligonkovaakademia.sk/api/noty/texty/addView.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: new URLSearchParams({ id: this.songDetail.id }),
        credentials: "include",
      });
    } catch (e) {
    ...
    // ✅ zistenie ID používateľa a nastavenie isFavorite
    try {
      ...
      try {
        const res = await fetch(
          "https://heligonkovaakademia.sk/api/noty/texty/favorite.php",
          {
            ...
            credentials: "include",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({ id: this.songDataInternal.id }),
          }
        );
