<template>
  <div v-if="produkt != null" class="oblubene naposledy-sledovane">
    <div class="nadpis">
      <p class="naposledy">Naposledy sledované:</p>
    </div>
    <div class="content">
      <div class="slider">
        <div class="box-kurz">
          <img
            v-if="produkt.difficulty == '2'"
            src="@/assets/images/obtiaznost/obtiaznostStredPokro.png"
            alt="Nazov not"
          />
          <img
            v-if="produkt.difficulty == '3'"
            src="@/assets/images/obtiaznost/obtiaznostPokro.png"
            alt="Nazov not"
          />
          <img
            v-if="produkt.difficulty == '1'"
            src="@/assets/images/obtiaznost/obtiaznostZac.png"
            alt="Nazov not"
          />
          <div @click="spustiVideo" class="button">
  ...
  <div v-else class="box-item oblubene naposledy-sledovane">
    <div class="nadpis">
      <p class="naposledy">Naposledy sledované:</p>
    </div>
    <div class="content">
      ...
      <div class="nazov-piesne">Žiadne sledované video</div>
    </div>
  </div>
</template>
    ...
    spustiVideo() {
      ...
      if (this.$store.state.user.isSubscribed) {
        this.$router.push({
          ...
          query: { cislo: this.produkt.id },
        });
      } else {
        this.$router.push({
          path: "/ucebna/clenstvo",
          query: { data: "predplatne" },
        });
      }
    },
  },
